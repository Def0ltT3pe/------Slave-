...
class NetworkScanner:
    ...
    def _enable_paste(self, widget):
        ...
        widget.bind("<Command-v>", self._paste_text)
    ...
    def create_ssh_window(self, ip, login, password):
        ...
        # Заголовок
        title_label = ttk.Label(main_frame, text=f"SSH сессия: {login}@{ip}", 
                               font=("Arial", 12, "bold"))
        title_label.pack(pady=(0, 10))
        
        # Терминал (текстовое поле с прокруткой)
        terminal_frame = ttk.Frame(main_frame)
        terminal_frame.pack(fill=tk.BOTH, expand=True)
        
        self.ssh_text = scrolledtext.ScrolledText(terminal_frame, wrap=tk.WORD, 
                                                 font=("Courier New", 10))
        self.ssh_text.pack(fill=tk.BOTH, expand=True)
        self.ssh_text.config(state=tk.NORMAL)
        ...
        send_button = ttk.Button(input_frame, text="Отправить", 
                                command=self.send_ssh_command)
        ...
        ttk.Button(button_frame, text="Очистить терминал", 
                  command=self.clear_ssh_terminal).pack(side=tk.LEFT)
        ttk.Button(button_frame, text="Разорвать соединение", 
                  command=lambda: self.disconnect_ssh(ssh_window)).pack(side=tk.RIGHT)
        
        # Запускаем SSH сессию
        self.ssh_output(f"Подключаемся к {login}@{ip}...\n")
        thread = threading.Thread(target=self.start_ssh_session, 
                                 args=(ip, login, password, ssh_window))
        thread.daemon = True
        thread.start()
    ...
    def start_ssh_session(self, ip, login, password, window):
        """Запуск SSH сессии с paramiko"""
        try:
            self.ssh_client = paramiko.SSHClient()
            self.ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            
            self.ssh_output(f"Устанавливаем соединение...\n")
            self.ssh_client.connect(ip, username=login, password=password, port=22, timeout=10)
            
            self.ssh_output("✓ SSH соединение установлено!\n")
            self.ssh_output("Вы можете вводить команды ниже.\n\n")
            
            # Выполняем базовые команды при подключении
            basic_commands = [
                "uname -a",
                "whoami", 
                "pwd",
                "ls -la",
                "echo '=== SSH сессия активна ==='"
            ]
    ...
    def send_ssh_command(self):
        """Отправка команды из интерфейса"""
        if not self.ssh_client:
    ...
    def _execute_command_thread(self, command):
        ...
        try:
            ...
            self.root.after(0, self._show_command_result, result)
        except Exception as e:
            self.root.after(0, self._show_command_result, f"Ошибка: {str(e)}")
    
    def _show_command_result(self, result):
        """Показ результата команды"""
        self.ssh_output(f"{result}\n")
    
    def ssh_output(self, text):
        """Вывод текста в SSH терминал"""
        def update_text():
            self.ssh_text.insert(tk.END, text)
            self.ssh_text.see(tk.END)
            self.ssh_text.update_idletasks()
        
        self.root.after(0, update_text)
    
    def clear_ssh_terminal(self):
        """Очистка SSH терминала"""
        self.ssh_text.delete(1.0, tk.END)
    
    def disconnect_ssh(self, window):
        """Разрыв SSH соединения"""
        if self.ssh_client:
            self.ssh_client.close()
            self.ssh_client = None
        window.destroy()
        self.status_var.set("SSH соединение закрыто")
